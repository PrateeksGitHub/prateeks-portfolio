...
export async function POST(request) {
  try {
    const resend = new Resend("re_SRf4duoN_HYjSEk4oFnN1JzBT2BKxmsz8");
    ...
    console.log(
      `Sending email from ${process.env.SENDER_EMAIL_ID} to ${email} and ${process.env.MY_EMAIL_ID} using API Key ${process.env.RESEND_API_KEY}`
    );
